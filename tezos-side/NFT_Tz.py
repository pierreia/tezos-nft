...
def test():
    ...
    cyber_token = Cyber_Token(FA2_config(single_asset=False,non_fungible=True), admin = admin.address, metadata = sp.big_map({"": sp.utils.bytes_of_string("tezos-storage:content"),"content": sp.utils.bytes_of_string("""{"name" : "Cyber Token"}""")}))
    ...
    scenario += cyber_token.mint(address = elon.address,
                                ...
                                metadata = sp.map(l = {
            # Remember that michelson wants map already in ordered
            "decimals" : sp.utils.bytes_of_string("%d" % 4),
            "name" : sp.utils.bytes_of_string('Your Bucks'),
            "symbol" : sp.utils.bytes_of_string('YTK'),
            "token_url": sp.utils.bytes_of_string('myurl.com'),
        }),
                                token_id = 1).run(sender = admin)
    scenario += cyber_token.burn(address = elon.address, token_id = 1).run(sender = admin)